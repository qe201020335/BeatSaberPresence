...
internal class TutorialPresenceManager : IInitializable, ILateDisposable
{
    ...
    internal TutorialPresenceManager([InjectOptional] IGamePause gamePause, PluginConfig pluginConfig,
    ...
    private string Format(string rpcString)
    {
        ...
        var totalTime = new TimeSpan(0, 0, (int)Math.Floor(audioTimeSyncController.songLength));
        ...
        formattedString = formattedString.Replace("{SongDurationSeconds}",
            Math.Floor(audioTimeSyncController.songLength).ToString());
